...
class FCNN_VAE(nn.Module):
    ...
    def forward(self, x):
        ...
        self.latent = tmp
        self.mu = tmp[:,:n_hidden//2]
        # how to encode the variance / standard deviation / log var ?
        self.logvar = tmp[:,n_hidden//2:]
    ...
    def decoder(self, latent):
        ...
        self.mu = latent[:,:n_hidden//2]
        # how to encode the variance / standard deviation / log var ?
        self.logvar = latent[:,n_hidden//2:]
        batch, dim = self.mu.shape
        var = torch.exp(self.logvar)
        std = torch.sqrt(var)
        eps = torch.randn_like(std)
...
def test_loop(test_x, test_y, model, loss_fn):
    size = len(test_x)
    test_loss = 0
